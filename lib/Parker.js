  ...
  class Parker extends EventEmitter {
    ...
    execute () {
      ...
      for (; blockIndex < blockLength; blockIndex++) {
        ...
        let taskLength = this.pOptions._status[currentLevel][blockIndex].tasks.length;
        taskIndex = 0;
        for (; taskIndex < taskLength; taskIndex++) {
          ...
          if (this.trace) {
            const statusObject = this.pOptions._status[currentLevel][blockIndex];
            console.log(`execute: ${statusObject.description} - p:${taskIndex}`);
          }
    ...
    _PromiseReport (status, fnIndex, thenLevel, orLevel) {
      ...
      // console.log({status, fnIndex, thenLevel, orLevel});
      var statusObject = this.pOptions._status[thenLevel][orLevel];
      // console.log(statusObject);
      if (status === 'pdone') {
        statusObject.success++;
        if (this.trace) {
          console.log(`done: ${statusObject.description} - p:${fnIndex}`);
        }
      } else {
        statusObject.errors++;
        if (this.trace) {
          console.log(`fail: ${statusObject.description} - p:${fnIndex}`);
        }
      }

      if (statusObject.success === statusObject.functions) {
      ...
      if (statusObject.success + statusObject.errors === statusObject.functions) {
